...
// WARNING: the sun never shines below here

Game Game::from_mxArray(mxArray* mgame) {
  Game cgame;
...

  {
    ...
    mex::bool_t* dboard = mxGetLogicals(mboard);
    mwSize nsubs = 3;
    mwIndex subs = {0, 0, 0};
...

  {
    ...
    if (!mex::is_int(mcheckpoints))
      mexErrMsgIdAndTxt("RR:BadArgument", "checkpoints should be int_t");

    mex::int_t* dcheckpoints = (mex::int_t*)mxGetData(mcheckpoints);
    mwSize nsubs = 2;
    mwIndex subs = {0, 0};
    Point x;
...

  {
    ...
    for (size_t i = 0; i < n; i++)
      cgame.add_robot(Robot::from_mxStruct(mrobots, i));
  }

  return cgame;
}

mxArray* Game::to_mxArray() {
    ...
    for (subs[0] = 0; subs[0] < dims[0]; subs[0]++)
      for (subs[1] = 0; subs[1] < dims[1]; subs[1]++)
        for (subs[2] = 0; subs[2] < dims[2]; subs[2]++)
          dboard[mxCalcSingleSubscript(mboard, nsubs, subs)] = board[subs[0]+1][subs[1]+1][subs[2]];
  }

  {
    mcheckpoints = mxCreateNumericMatrix(checkpoints.size(), 2, mex::int_t_classid, mxREAL);
    dcheckpoints = (mex::int_t*)mxGetData(mcheckpoints);
    mwSize nsubs = 2;
    mwIndex subs = {0, 0};
    for (subs[0] = 0; subs[0] < checkpoints.size(); subs[0]++)
      for (subs[1] = 0; subs[1] < 2; subs[1]++)
        dcheckpoints[mxCalcSingleSubscript(mcheckpoints, nsubs, subs)] = checkpoints[i].x[subs[1]];
  }
