...
height = 0.5;
width = height*scale;
images.conveyor.plain = generate_conveyor_images(imread('images/conveyorEast.png'), height, width, RR.directions.asnumbers);

scale = 112/114;
height = 0.7;
width = height*scale;
images.conveyor.cw = generate_conveyor_images(imread('images/conveyorEastClockwise.png'), height, width, RR.directions.asnumbers);

scale = 126/118;
height = 0.7;
width = height*scale;
images.conveyor.ccw = generate_conveyor_images(imread('images/conveyorEastCounterclockwise.png'), height, width, RR.directions.asnumbers);
...
for y = 1:board_height(board)
    for x = 1:board_width(board)
        ...
        for feature = features(board_has_feature(board, ij, features))
            dir = feature - RR.features.conveyor_east;
            image = images.conveyor.(conveyor_type){1+dir};
